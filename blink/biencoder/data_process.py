...
def process_mention_data(
    ...
    types = pickle.load(open("/root/autodl-tmp/data/release/zeshel/documents/all_type", 'rb'))
    ...
    for idx, sample in enumerate(iter_):
        ...
        label_doc_id = sample["label_doc_id"]
        category_label = types[label_doc_id]
        category_label_idx = np.zeros(10331)
        mention_label_idx = np.zeros(10331)
  
        for mention_la in category_label:
            mention_label_idx[mention_la] = 1
                
        for category in category_label:
            category_label_idx[category] = 1
        ...
        record = {
            ...
            "label": label_tokens,
            "label_idx": [label_idx],
            "mention_label": mention_label_idx,
            "category_label": category_label_idx
        }
    ...
    mention_label = torch.tensor(
        np.array(select_field(processed_samples, "category_label")), dtype=torch.long,
    )
    ...
    if use_world:
        data["src"] = src_vecs
        tensor_data = TensorDataset(context_vecs, cand_vecs, src_vecs, label_idx, mention_label, category_label)
    else:
        tensor_data = TensorDataset(context_vecs, cand_vecs, label_idx, mention_label, category_label)
