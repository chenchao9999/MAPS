...
def encode_candidate(
    ...
    entity_latent_list = None
    for step, batch in enumerate(iter_):
        cands = batch
        cands = cands.to(device)
        cand_encode, entity_latent = reranker.encode_candidate(cands)
        if cand_encode_list is None:
            cand_encode_list = cand_encode
        else:
            cand_encode_list = torch.cat((cand_encode_list, cand_encode))
        if entity_latent_list is None:
            entity_latent_list = entity_latent
        else:
            entity_latent_list = torch.cat((entity_latent_list, entity_latent))
    cand_encodes = (cand_encode_list, entity_latent_list)
...
def load_or_generate_candidate_pool(
    ...
    if candidate_pool is None:
        ...
        entity_desc_list = load_entity_dict_zeshel(logger, params)
        candidate_pool = get_candidate_pool_tensor_zeshel(
            entity_desc_list,
            tokenizer,
...
def main(params):
    ...
    if not os.path.exists(os.path.join("./loaddata/", 'test_data.pkl')):
        ...
        pickle.dump(test_data, open("./loaddata/test_data.pkl", 'wb'), -1)
    else:
        test_data=pickle.load(open("./loaddata/test_data.pkl", 'rb'))
        test_tensor_data=TensorDataset(test_data["context_vecs"], test_data["cand_vecs"],test_data["src"],test_data["label_idx"],test_data["mention_label"],test_data["category_label"])
