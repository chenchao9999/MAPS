...
def prepare_instance_bert(filename, args, max_length):
    ...
    for i in range(len(datas)):
        # print(datas[i])
        mention = datas[i]["mention_span"]
        context =" ".join( datas[i]["left_context_token"])+mention+" ".join(datas[i]["right_context_token"])
        # labels = datas[i]["labels"] + datas[i]["fine_labels"] + datas[i]["ultra_fine_labels"]
        labels=datas[i]["y_str"]
        ...
        for label in labels:
            for j, la in enumerate(all_types):
                if la == label:
                    ...
                    if j<130:
                      label_coarse[j]=1
                      label_coarse_index.append(j)
        
        if label_coarse_index==[]:
            label_coarse[130]=1
...
def prepare_instance_generate(filename, args, max_length):
    ...
    # prompt=["[PROMPT]","[PROMPT]","[PROMPT]"]+["[SEP]"]
...
def my_collate_bert(x):
    ...
    labels_coarse = [x_['labels_coarse'] for x_ in x]
    ...
    return inputs_id, segment_ids, masks, labels,labels_coarse
