...
def main(params):
    ...
    neg_samples = utils.read_dataset("train_pair_neg_1", params["data_path"])
    ...
    if not os.path.exists(os.path.join("./loaddata/", 'neg_data_1.pkl')):
        ...
        pickle.dump(neg_data, open("./loaddata/neg_data_1.pkl", 'wb'), -1)
        # torch.save(train_data, "./loaddata/train_data.pt")
        # torch.save(train_data, "./loaddata/train_tensor_data.pt")
    else:
        neg_data=pickle.load(open("./loaddata/neg_data_1.pkl", 'rb'))
        
    train_tensor_data=TensorDataset(train_data["context_vecs"], train_data["cand_vecs"],train_data["src"],train_data["label_idx"],train_data["mention_label"],train_data["category_label"],neg_data["context_vecs"], neg_data["cand_vecs"],neg_data["src"],neg_data["label_idx"],neg_data["mention_label"],neg_data["category_label"])
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            batch = tuple(t.to(device) for t in batch)
            context_input, candidate_input, _, _, mention_label, category_label ,context_input_neg, candidate_input_neg, _, _, mention_label_neg, category_label_neg = batch
            context_input=torch.cat((context_input,context_input_neg),dim=0)
            candidate_input=torch.cat((candidate_input,candidate_input_neg),dim=0)
            mention_label=torch.cat((mention_label,mention_label_neg),dim=0)
            category_label=torch.cat((category_label,category_label_neg),dim=0)
