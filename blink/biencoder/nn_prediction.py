...
def get_topk_predictions(
    ...
    if silent:
        ...
        iter_ = tqdm(train_dataloader,mininterval=20)
    ...
    for step, batch in enumerate(iter_):
        ...
        context_input, _, srcs, label_ids, _, _ = batch
        src = srcs[0].item()
        scores, _ = reranker.score_candidate(
            context_input, 
            None, 
            cand_encs=cand_encode_list[src][0].to(device),
            latent_encs = cand_encode_list[src][1].to(device)
        )
    ...
    nn_first = torch.LongTensor(nn_first)
    nn_data = {
